---
title: Perception
description: "感知层负责记录结构化的内容结果、按维度聚合并生成有边界的洞察摘要，让后续 Agent 决策参考真实运营数据，而不是凭感觉判断。它是分析反馈层，不保证内容本身正确，也不强依赖完整历史注入，适合关注内容效果与增长指标的团队使用。，数据维度可自定义。"
---

# Perception 感知层

> **快速答案：** Perception 记录结构化内容结果，按维度生成统计洞察，并输出有边界的摘要供后续决策使用。它是分析和反馈层，不保证内容一定正确，也不代表每次会话都会注入完整历史。

## 一个有边界的反馈循环

记录结果，积累足够样本后生成洞察，只在下一步决策需要时请求摘要。用配置限制摘要大小，避免上下文被历史噪声淹没。
...
### Perception 会自动发布或优化内容吗？

不会。它只记录结果并生成统计摘要；发布、编辑和外部操作仍属于独立流程，需要各自的批准与验证。

### 每次任务会注入多少历史？

摘要受 `PERCEPTION_MAX_CHARS` 等配置限制。遵循 pull-based 上下文规则，只在下一步需要时读取必要材料。

## 官方文档

配合阅读[ContextDB](contextdb.md)、[工作流策略](workflow-policy.md)和[故障排查](troubleshooting.md)。
