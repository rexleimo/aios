---
title: debug-hub
description: "MCP 네이티브 디버그 로그 서비스입니다. 코딩 에이전트가 스스로 런타임 로그를 검색하고 실행 경로를 추적하며 실패 원인을 찾아 수정안을 제시합니다. 로그를 직접 붙여넣거나 증상을 다시 설명할 필요가 줄고, 지난 로그는 세션, 기간, 키워드로 검색할 수 있습니다."
---
