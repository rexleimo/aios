---
title: 변경 로그
description: "AIOS 전체 릴리스 이력입니다. 각 버전에서 무엇이 바뀌었고 업그레이드 시 무엇을 주의해야 하며 호환성 깨지는 변경이 어디 있는지, 그리고 해당 기능 문서로 가는 입구를 제공합니다. 업그레이드 전에 영향 범위를 바로 확인할 수 있습니다."
---

# 변경 로그

## v6.0.0 (2026-09-21) — 엔지니어링 표준 내장: 생성 코드의 성장

- **엔지니어링 표준이 프레임워크 1급 시민으로**: 새 스킬 `aios-engineering-standards`가 고전 서적 목록을 모든 코드 생산 작업의 Definition of Done으로 바꾼다 — Clean Architecture 의존 규칙과 경계 규율, Ousterhout의 깊은 모듈, Clean Code 명명/함수/오류 처리 규칙, DRY와 직교성, 테스트 기준, 도구 체인 기준(lint + pre-commit + CI + 구조화 로그), 그리고 중요한 변경의 ADD 기록. 참조: [엔지니어링 표준](engineering-standards.md).
- **능력 체인에 통합**: `aios-workflow-router`가 `rex-implement` / `rex-refactor-hardening` / `rex-code-review` / `rex-design` 전에 이 표준을 로드하고, `pre-edit-safety-gate`가 선택한 변경 형태를它에 맞춰 확인한다. 우회 가능한 새 문이 아니라 — 품질 기준은 기존 증거 기반 체인 위를 달린다. 참조: [v6.0.0 릴리스 글](/blog/ko/2026-09-v600-engineering-standards/).
- **문서와 참고 자료**: 엔지니어링 표준 페이지를 en/zh/ja/ko로 제공. 4단계 능력 모델, 9권 서적 목록과 핵심 가치, 무료 합법 자료, "서적 원리 → AIOS 메커니즘" 매핑 포함.
...
- **headless와 정직한 한계 기록**: team/harness는 `-p` 출력 모드에 `--yolo`로 Qoder를 구동. 모델 라우팅은 `own`(모델이 계정에 묶여 대화형 `/model`로 선택, 검증된 headless `--model`이 없어 AIOS는 엔드포인트를 중계하지 않음)이고 호스트 능력은 L2로 한계를 함께 기록한다. 참조: [Qoder 클라이언트 블로그 글](/blog/ko/2026-09-v520-qoder-client/).
...
- **플랜 상태 동기화가 더 이상 먼저 나서지 않습니다**：`syncPlanWithIterationOutcome`은 sync할 때마다 `markPlanTaskInProgress`를 호출하지 않습니다. 서브에이전트 런타임이 명시적 task id 없이 성공을 보고하면 다음 pending 작업이 강제로 `in_progress`가 되던 문제를 해결했습니다；이제 sync는 증거만 기록하고 명시적인 `taskId`가 있을 때만 동작하며, `in_progress`는 오직 harness loop만 소유합니다. 죽은 코드 `hasCommitEvidence` 헬퍼를 제거하고 `hasTargetFileChanges`의 절대/상대 경로 매칭도 수정했습니다.
- **WorkBuddy가 제1급 AIOS 클라이언트로**：네이티브 워크플로/스킬 생성, `~/.workbuddy/mcp.json`에 기록되는 MCP 설정, 완전한 24/24 스킬 동기화, 그리고 동봉된 `codebuddy` CLI를 통한 solo-harness 구동까지 모두 연결됐습니다. `aios harness run --provider workbuddy`가 프로바이더를 해석해 실행합니다.

### 업그레이드 안내

- `aios update`로 업데이트하세요. 설정 마이그레이션은 필요 없습니다.
- `codebuddy` 바이너리는 기본적으로 PATH에 없습니다. 다음을 셸 설정에 추가하고 클라이언트를 재시작하세요.
...
- `aios work`가 활성 구조화 플랜에서 병렬 워크 아이템을 분해합니다. 대상 플랜 태스크는 의존성·소유 경로(`targets` + `allowedWrites`)·수용 기준을 유지한 워크 아이템이 됩니다. `;` 구분 `--context`는 플랜 없는 실행의 폴백으로 유지됩니다.
- 디스패치 후 보고서가 플랜 기반 분해를 유지하므로 최상위 `workItems`는 항상 실제 실행된 디스패치 플랜과 일치합니다.
- 새 정식 `aios-work-dispatch` 스킬: 언제 병렬 디스패치에 들어갈지(planned disposition, 독립 아이템 2개 이상, 파일 소유권 중복 없음, 엄격한 순서 없음), 분해 표현법, 미리보기/승인 경계. `aios-workflow-router`는 병렬 가능한 플랜형 작업을 디스패치 스킬로 라우팅합니다.

### 업그레이드 안내

- `aios update`로 업데이트. 설정 마이그레이션 불필요. 프로젝트나 설치 디렉터리를 이동한 후 MCP 서버가 "connection closed"가 되면 새 프로젝트 루트에서 `aios update` 또는 `aios internal browser mcp-migrate`를 다시 실행하고 클라이언트를 재시작하세요([문제 해결](troubleshooting.md) 참조).
...
- `normalizeEvidenceRefs()`를 추가했습니다: evidence ref에 프로토콜 프리픽스(`artifact:`, `receipt:` 등)가 필수이며 TODO/TBD/placeholder는 거부됩니다. Wayfinder, Planning, Requirements 전체 산출물에 적용됩니다.
