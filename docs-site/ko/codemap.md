...
description: "Tree-sitter 기반 구조 지식 그래프입니다. 코딩 에이전트가 판단할 때마다 호출자, 의존 대상, 테스트 커버리지, 영향 반경을 바로 볼 수 있습니다. 코드를 반복해서 grep할 필요가 없고, 명령 하나로 여러 클라이언트가 같은 코드 맵을 공유하며 변경 영향 범위가 한눈에 보입니다."
---
