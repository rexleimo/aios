...
どちらの面もアクションを**狭める**ことしかできません。内容を生成することも、拒否された進行を許可に変えることもできません。判定側に到達できない場合の既定は hold です。ゲートはあなたが意図して有効にしたものだからです。ベンダー障害を通したい場合は `onJudgmentError` を `"allow"` にしてください。

rex ステージゲートは host 側のポリシーです。rex サブモジュールはその存在を知らないため、ゲートを無効のままにすれば以前の振る舞いが完全に保たれます。
