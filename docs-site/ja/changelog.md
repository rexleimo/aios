---
title: 変更履歴
description: "AIOS の全リリース履歴: 各バージョンで何が変わり、アップグレード時に何に注意し、破壊的変更がどこにあるか、そして該当機能のドキュメントへの入口を示します。アップグレード前にここを見れば影響範囲をすぐ確認でき、ある機能がどのバージョンで入ったかも遡れます。"
---
...
- **aios-shell MCP が長コマンドでフリーズしなくなりました**：shell server と stdio proxy が JSON-RPC を並行処理するようになり、コマンド実行中も ping・キャンセル・その他のリクエストに即時応答します。`notifications/cancelled` は requestId 単位で実行中のコマンドをタイムアウト待ちではなく即時終了させ、Windows では `taskkill /T /F` でプロセスツリー全体を掃除し、stdin クローズ時にはすべての実行中コマンドを破棄します。
- **aios-shell プロキシチェーンを維持**：`aios-mcp-proxy.mjs` が `_meta.aios` 観測メタデータとローカル ref ストレージを引き続き提供します（RTK/Caveman は引き続きクライアント側唯一の出力圧縮）。`SHELL_TOOL.description` の古い "compression via AIOS MCP proxy" 記述は削除しました（プロキシは出力をそのまま転送し、圧縮しません）。
- **MCP server 起動タイムアウトの安全網**：生成される Codex `config.toml` の server セクションに `startup_timeout_sec`（60/30/30）を付与し、OpenCode `opencode.json` マイグレーション時は欠落があれば `experimental.mcp_timeout: 90000` を注入します。
- **要件明確化のトリガーを正規表現から LLM の意味判断へ変更**：旧正規表現（`MISSING_REQUIREMENTS_PATTERN` / `VAGUE_BEHAVIOR_PATTERN` / `VAGUE_GOAL_PATTERN`）は明示的な言い回ししか検出できず、具体的な機能名はあるが受け入れ基準・範囲・成功定義が欠けている「意味レベルの曖昧さ」を拾えませんでした。現在 `derive-facts.mjs` は文言から `ACCEPTANCE_CRITERIA_MISSING` を生成せず、`requirementsCapability.activate()` は `grill`/`spec` の明示インテントまたはドメイン語彙の曖昧さ observation でのみ発火します。`rex-requirements` スキルは**実行中に埋め込まれた問いかけ（grilling）**として書き直しました：一度に一問・推奨回答付き・3 ラウンドで収束し、本当の決定ポイントに当たったときだけ質問します——冒頭の尋問ゲートではありません。description は二重トリガー（LLM が曖昧な依頼を自己判断でトリガー、または rex-harness がアクティベート）です。
- **ステージ境界での挿入**：`advanceSoftwareWorkflow` は各 Capability 完了後に再選択するため、実行途中で決定ポイントに当たっても次のステージ境界で要件明確化を挿入でき、明確化完了後に元の Capability を再開します。

### アップグレード手順

- `aios update` で更新してください。設定の移行は不要です。
- 更新後は opencode/codex クライアントを再起動し、新しい shell server とプロキシを反映させてください。
- `rex-code-review` に**シナリオベースのサブエージェント受入モード**（正常/境界/異常シナリオのマトリクスと証拠収集）を追加しました。`references/acceptance-scenario-matrix.md` を参照してください。
...
- `aios work` がアクティブな構造化プランから並行ワークアイテムを分解するようになりました。対象プランタスクは依存関係・所有パス（`targets` + `allowedWrites`）・受入条件を保持したワークアイテムになります。`;` 区切りの `--context` はプランなし実行のフォールバックのままです。
- ディスパッチ後レポートはプラン駆動の分解を保持するため、トップレベルの `workItems` は常に実行されたディスパッチプランと一致します。
- 新しい正規 `aios-work-dispatch` スキル：いつ並行ディスパッチに入るか（planned disposition、独立アイテム 2 つ以上、ファイル所有の重複なし、厳密な順序なし）、分解の表現方法、プレビュー／承認境界。`aios-workflow-router` は並行可能なプラン型作業をディスパッチスキルへルーティングします。

### アップグレード情報

- `aios update` で更新。設定移行は不要。プロジェクトやインストールディレクトリを移動した後に MCP server が "connection closed" になる場合は、新しいプロジェクトルートで `aios update` または `aios internal browser mcp-migrate` を再実行し、クライアントを再起動してください（[トラブルシューティング](troubleshooting.md)参照）。
...
- `agents smoke --live` が JSON 出力コントラクトに従うクライアント（例：Codex）で失敗しなくなりました。プローブプロンプトはコントラクトを明示的に上書きして ACK のみを要求し、ACK 検出は JSON ラップされた応答も許容します。post-receive 圧縮証明は `minRawBytes` 以上の出力にのみ適用され（短い出力は設計上インライン化）、空の圧縮 refs でエビデンス記録がクラッシュすることもありません。
- ハードコードされた 30 秒のプローブタイムアウトは `AIOS_AGENT_SMOKE_TIMEOUT_MS` と `agents smoke --timeout-ms <ms>` に置き換わりました（デフォルトは 60 秒に引き上げ）。
- プローブは一時的な遅延応答に対してタイムアウトを自動エスカレーションして再試行します（60s → 120s → 240s）。すべて使い切るまで blocked にはなりません——一度の遅いコールドスタートやモデル待ちでエージェントが恒久的にワークフロー無効になることはありません。最終的な blocker メッセージには回復コマンドが含まれます。

### アップグレードノート

- 既存インストールは `aios update` で更新できます。これまでに「command 無効 / workflow カード」状態（live smoke エビデンスのドリフトでエージェントが blocked）に遭遇した場合、`aios agents smoke --live --client <名前> --timeout-ms <ms>` を再実行して v2 エビデンスを再生成してください。データ移行は不要です。
...
- 既存インストールは本リリース導入後、`aios update` で正常に更新できます。修正自体はリリース資産に含まれています。
