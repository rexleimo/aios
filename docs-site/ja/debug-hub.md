---
title: debug-hub
description: "MCP ネイティブのデバッグログサービスです。coding agent が自分でランタイムログを検索し、実行パスを追跡し、失敗原因を特定して修正案を出せます。導入後はログを手で貼り付けたり現象を説明し直したりする必要が減り、過去のログはセッション、期間、キーワードで検索できます。"
---
