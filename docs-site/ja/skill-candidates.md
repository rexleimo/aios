...
description: "失敗したセッションからスキル改善パッチを発見、レビュー、適用する方法を説明します。候補がどう検出され、どうレビューし、どう安全に適用・ロールバックするかを理解すれば、agent の失敗を積み上げ可能な改善に変えられます。正式なスキルに入るのはレビューと検証を通過した候補だけです。"
---
