...
Any prompt, skill, or script that guides the model follows three rules (full text: `docs/prompt-authoring-norms.md`):

1. **Contract first**: declare input preconditions and a structured output contract; never branch on keywords/regex matched against user text — require an explicit `intent`/`task-type` field and let policy look it up.
2. **Self-report, don't guess**: semantic judgments (done? stuck? read-only?) are fields the model reports in structured output (ReAct-style); code only counts/validates them.
3. **Hard gates are protocols**: guarantees (tests pass, evidence exists, budget left) are enforced by runtime verification (exit codes/hashes in evidence envelopes), never by prompt politeness and never by text heuristics.

Code may verify objective facts; code must never guess model or user intent.

## Project Structure & Module Organization
This repository is AIOS — a local-first orchestration control plane that gives coding clients (codex / claude / gemini / opencode / hermes / grok / workbuddy / pi / zcode / qoder) project memory, adaptive workflow routing, multi-agent teams, resumable long-running tasks, and verifiable evidence. The browser MCP is a legacy component inside it, not the project's center.
...
- `mcp-server/`: TypeScript MCP servers, including ContextDB and a legacy Playwright browser runtime (`mcp-server/src/index.ts` entry, `mcp-server/src/browser/` runtime).
- `scripts/run-local-browser-mcp.mjs`: legacy launcher for the repository-local Node/Playwright browser MCP.
- `config/`: runtime configuration such as `browser-profiles.json` and safety-related settings.
- `.aios/memo/`: Git-friendly project memo records generated by `aios memo`.
...
- 记忆/会话命令统一走全局安装入口 `~/.aios/bin/aios`（如 `~/.aios/bin/aios memo search "..."`、`~/.aios/bin/aios session start --json`）。不要执行仓库相对路径的 `node scripts/aios.mjs`——它只在 AIOS 源码仓库存在，且会绕过已安装运行时；MCP `aios-memory` 工具仅在该 server 已配置时可用，不可用时一律退回上述 CLI。
- 新会话第一条消息：先 `memory_recall`（MCP `aios-memory` 工具，或 `~/.aios/bin/aios memo search`）检索相关记忆再开工。
- 用户说 继续 / 接着做 / resume：先 `memory_recall` + `~/.aios/bin/aios session start --json` 恢复上下文，不凭空猜上次进度。
