...
以下旁路任务不参与强模型路由，直接使用低成本模型；超限时沿 fallback 链降级，不升级：

| 旁路任务 | 钉死模型 | 超限 fallback |
|----------|----------|---------------|
| 对话摘要、标题生成 | **DeepSeek-V4** | GLM-5.1 → 默认实现模型 |
| 记忆抽取（memo extraction） | **DeepSeek-V4** | GLM-5.1 → 默认实现模型 |
| 常规文档、注释、changelog 草稿 | **Claude Sonnet 4.6** | DeepSeek-V4 → 默认实现模型 |
| 例行评审（非安全） | **Claude Sonnet 4.6** | 强模型仅在 high 风险声明时 |

只有显式的硬能力约束（浏览器、1M 上下文、安全审计、生产恢复）才能打破钉死。任务类型由你语义判断后声明，程序不从文本关键词反推。

## 预算声明

每次调度显式声明预算字段，随路由请求一起记录，供后续熔断消费：

- `budget`: 本任务 token/费用上限（数字 + 单位，如 `200k tokens`）。
- `quota_scope`: 配额归属（单任务 / DAG 分支 / 整轮 run）。
- 超支行为（二选一并声明）：`downgrade`（降档弱模型继续）或 `fail-fast`（立即停并交接）。

无声明即无熔断——这是确定性语义，不是程序替你省钱。预算只做客观计数（token/费用），不做语义判断。
