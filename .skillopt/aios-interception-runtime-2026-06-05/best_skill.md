...
- If validation reports skipped tests, classify them before drawing token-compression conclusions.
- Platform-gated skips (for example Windows-only PowerShell or installer smoke tests on macOS/Linux) are not evidence that turn compression was skipped.
- Token-compression acceptance must cite the focused interception tests and `turn_compression_matrix` metrics, not the aggregate skip count alone.
- Do not hide skips: report exact skip count, why they skipped, and whether they are related to `pre_send` / `post_receive`.
