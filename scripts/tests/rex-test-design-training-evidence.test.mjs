...
const TRAINING_ROOT = path.join(ROOT, '.skillopt', 'rex-test-design-2026-07-17');

async function readJson(...segments) {
  return JSON.parse(await readFile(path.join(...segments), 'utf8'));
}

test('rex-test-design 在留出集硬分饱和时保留基线', async () => {
  const [train, validation, gate, state, canonical] = await Promise.all([
    ...
    readFile(path.join(ROOT, 'rex-harness', 'skill-sources', 'rex-test-design', 'SKILL.md'), 'utf8'),
  ]);
