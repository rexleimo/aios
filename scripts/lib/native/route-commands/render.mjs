...
export function buildRoutePromptBody({ rootDir, client, route, placeholder }) {
  ...
  if (route === 'single') {
    return [
      ...
      'Run the AIOS workflow policy adapter first: `node scripts/aios.mjs plan auto-gate --task "<task arguments>" --client ' + client + '`. It selects `direct`, `guarded`, or `planned` before a plan or skill is selected.',
  ...
  if (route === 'plan') {
    return [
      ...
      '7. When finishing later, run the selected verification process and `node scripts/aios.mjs plan set-status --status done`.',
      AIOS_ROUTE_COMMAND_END,
    ].join('\n');
  }
