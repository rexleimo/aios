...
export function buildRoutePromptBody({ rootDir, client, route, placeholder }) {
  ...
  if (route === 'single') {
    return [
      ...
      'Run the AIOS workflow policy adapter first: `aios plan auto-gate --task "<task arguments>" --client ' + client + '`. It selects `direct`, `guarded`, or `planned` before a plan or skill is selected.',
      '`direct` has no persistent plan or skill chain. `guarded` uses `pre-edit-safety-gate` and focused verification. Only `planned` creates or reuses an AIOS plan and invokes the minimum relevant playbook.',
  ...
  if (route === 'plan') {
    return [
      ...
      '',
      'This slash command creates or reuses one explicit **planned work item**. Let Rex select the current Provider; do not preload a fixed skill chain.',
      '',
      'Required steps:',
      '1. Evaluate the workflow policy and read the current Rex Capability Command before choosing any provider.',
      ...
      '',
      '```bash',
      'aios plan start --title "<short-title>" --task "<task arguments>" --client ' + client,
      '```',
      '',
      '4. If host Plan mode was used, **mirror the outcome into that file** before implementation.',
      '5. Invoke only the Provider returned by Rex and submit its required evidence before requesting the next stage.',
      '6. Do not implement code until the plan artifact and current Rex command exist, unless the user explicitly asked to skip planning.',
      '7. When finishing later, run the selected verification process and `aios plan set-status --status done`.',
      AIOS_ROUTE_COMMAND_END,
    ].join('\n');
  }
  ...
  return [
    ...
    'This route is work-item-scoped: use it only for one explicit `planned` work item. Do not create a replacement plan or re-run a global bootstrap in the routed worker.',
