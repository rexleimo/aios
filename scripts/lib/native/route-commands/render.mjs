...
export function buildRoutePromptBody({ rootDir, client, route, placeholder }) {
  ...
  if (route === 'plan') {
    return [
      ...
      '',
      'This slash command creates or reuses one explicit **planned work item**. Select the smallest relevant playbook; do not invoke `using-superpowers` as a global bootstrap.',
      '',
      'Required steps:',
      '1. Use `brainstorming` only when scope is unclear, then use `writing-plans` for this planned work item.',
