...
export function renderOpenCodeStrictPrimaryAgent() {
  return [
    ...
    '',
    'Runtime budgets:',
    '- Finish the objective in this session: keep executing verifiable steps until every acceptance criterion is met, the user asks you to stop, or you are genuinely blocked. Do not stop at a partial status report and do not hand work to a "next session".',
    '- Treat one user request as one work item. Only stop early when the work is complete or you hit a real blocker; record the exact blocker instead of a to-do list for later.',
