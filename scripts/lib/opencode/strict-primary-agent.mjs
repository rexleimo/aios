...
export function renderOpenCodeStrictPrimaryAgent() {
  return [
    ...
    '- `direct`: answer or inspect without a persistent plan or a Superpowers chain.',
    '- `guarded`: make a small clear change only after `pre-edit-safety-gate`, then run focused verification.',
    '- `planned`: create or reuse one work-item plan and follow the selected process skill exactly; do not paraphrase or inline its workflow.',
    '- Do not invoke `using-superpowers` as a global bootstrap.',
    ...
    '',
    'OpenCode skill name resolution for a policy-selected playbook:',
