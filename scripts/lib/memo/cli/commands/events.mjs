...
export async function handleMemoListCommand({ argv, workspaceRoot, activeSpace, io }) {
  ...
  const rows = await loadMemoRows({ workspaceRoot, activeSpace, limit: flags.limit });
  if (rows.length === 0) {
    io.log('(none)');
    return true;
  }
...
async function loadMemoRows({ workspaceRoot, activeSpace, limit }) {
  const storageApi = await loadMemoStorageApi();
  const storage = await getActiveMemoStorage(workspaceRoot, storageApi);
...
async function searchMemoRows({ workspaceRoot, activeSpace, query, limit }) {
  const storageApi = await loadMemoStorageApi();
  const storage = await getActiveMemoStorage(workspaceRoot, storageApi);
