...
export function usage() {
  console.log(`Usage:
  node scripts/contextdb-shell-bridge.mjs --agent <codex-cli|claude-code|gemini-cli|opencode-cli|hermes-agent|grok-build|workbuddy-agent|pi-coding-agent|zcode-cli|qoder-cli> --command <codex|claude|gemini|opencode|hermes|grok|codebuddy|pi|zcode|qoder> [--cwd <path>] [-- <args...>]
...
export function validateOptions(opts) {
  ...
  if (!validAgents.has(opts.agent)) {
    throw new Error(`--agent must be one of: ${[...validAgents].join(', ')} (if you expect a newer client like pi-coding-agent, the AIOS runtime is stale — run: aios update --self-update)`);
  }

  if (!validCommands.has(opts.command)) {
    throw new Error(`--command must be one of: ${[...validCommands].join(', ')} (if you expect a newer command like pi, the AIOS runtime is stale — run: aios update --self-update)`);
  }
