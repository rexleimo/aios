...
# v5.14.0：LoopX 控制面 + 孤儿进程终结

v5.14.0 是 v5.11.0 之后的首个发布版本。除 v5.12.0 记忆面与 Pi 客户端外，它带来了长任务 harness 的控制面采纳，以及来自我们自己使用反馈的最后两处卡死修复。
...
v5.14.0 让每一轮 detached 启动，三段式清理整棵进程组（SIGTERM 组、宽限、SIGKILL 组、存活校验）。SIGKILL 后还活着的树直接停机，绝不并发。验证任务就是长？`--turn-timeout-ms` 把上限显式化。
...
从 v5.14.0 release 资产里取 installer 脚本重跑即可；changelog 提供中英日韩四语。
