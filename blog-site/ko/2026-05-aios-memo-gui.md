...
aios memo GUI는 기존 ContextDB 데이터를 읽어옵니다. 이미 AIOS가 설정되어 있고 프로젝트에 `.contextdb-enable`이 있다면, 데이터가 준비되어 있습니다.
...
이미 AIOS에서 ContextDB를 사용하고 있다면, 데이터가 준비되어 있습니다. aios memo GUI를 열고 탐색을 시작하세요.

AIOS가 처음이신가요? [퀵스타트 가이드](https://cli.rexai.top/ko/getting-started/)로 시작하여 프로젝트에서 ContextDB를 실행하고, 돌아와서 에이전트의 기억을 시각화하세요.
