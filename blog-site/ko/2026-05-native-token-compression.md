---
title: "네이티브 Token 압축: RexCLI가 RTK나 Caveman을 설치하지 않는 이유"
date: 2026-05-12
tags: ["AIOS", "token compression", "ContextDB", "skills", "RexCLI"]
description: "RexCLI는 ContextDB 입력 압축, 브라우저의 압축 읽기, CLI 출력 범위 제한, 출력 압축 스킬을 자체 구현하며 경쟁 도구에 의존하지 않습니다."
---

# 네이티브 Token 압축: RexCLI가 RTK나 Caveman을 설치하지 않는 이유

Token 비용은 단순한 과금 문제가 아닙니다. 안정성 문제이기도 합니다.

긴 AI 코딩 세션은 전체 로그, 반복 stack trace, 페이지 boilerplate, 장황한 상태 보고 때문에 쉽게 망가집니다. 가장 쉬운 지름길은 경쟁 token 도구를 설치하는 것입니다. RexCLI는 그 방식을 선택하지 않았습니다.

RTK 스타일의 입력 필터링과 Caveman 스타일의 짧은 출력이라는 아이디어만 참고하고, 실제 동작은 AIOS 안에서 자체 구현합니다.

## 무엇이 바뀌었나

RexCLI는 token 절감을 두 개의 네이티브 레이어로 나눕니다.
...
- 명령 동작이 RexCLI 통제 밖에서 바뀔 수 있습니다.
...
RexCLI의 token 절감은 이제 네이티브입니다. 입력 압축은 ContextDB와 브라우저 workflow가 담당하고, 출력 압축은 AIOS skill이 담당하며, 경쟁 도구 설치 단계는 없습니다.
