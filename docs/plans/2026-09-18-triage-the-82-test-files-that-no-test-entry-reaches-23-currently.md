...
- `work-fixture-refresh` (DONE, 2026-09-21 third pass, owner approved A+C): re-certified all 7 involved skills with base=HEAD — all `accepted` (deterministic local scoring, no LLM/network). Rewrote the 5 tests to assert via the production `verifySkillTrainingGate` that current Skill content is covered by tracked V2 evidence with matching `acceptedSkillHash` (shared helper `scripts/tests/fixtures/skill-training-freshness.mjs`); batch-invalid keeps its rejection intent hermetically (accepted verifies, then a `summary.trainHard` tamper on `candidate.scored.json` is blocked). Decisive findings: `.skillopt/` is gitignored + zero-tracked (old tests could never run in CI — the real reason they stayed unwired); the gate re-derives the baseline from `base`, so only HEAD-produced certs verify, and `statePath` must be absolute. Per-file: planning 1/1, strict-tdd 1/1, test-design 1/1, v2-case-selection 1/1 (4 skills), batch-invalid 1/1. deps: work-triage.
...
- `work-wire` (DONE for the fixed families, 2026-09-21 second pass): the 8 fixed families plus the already-green `interception-mcp-proxy` — 9 files — are wired into the `regression` suite in `scripts/test-suites.json`; snapshot refreshed (`knownUnwired` 82 → 73, exactly the wired count, W3 verified). Verification: wiring guard 3/3, full regression `files=124, 1404 tests, 1396 pass, 0 fail`. THIRD PASS same day (A3 resolved): the 5 rewritten rex fixture files wired as well — `knownUnwired` 73 → 68 (W3 verified, wiring guard 3/3); full regression `files=129, 1409 tests, 1401 pass, 0 fail, 8 skipped` — exact +5/+5 delta.
- `work-snapshot-refresh` (DONE for the wired count): baseline shrank by exactly 9, enforced by guard W3.
- still open: nothing from this triage — all 23 failures across 14 files are adjudicated and fixed; the only reds that ever existed are now green and wired. Remaining unwired (68) are files no entry reaches that were never failing, plus the rex-harness surface still under owner review.
- blocked: nothing. work-fixture-refresh unblocked by the owner-approved A+C decision and completed.
