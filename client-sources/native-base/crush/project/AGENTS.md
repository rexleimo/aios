...
Crush work in this repository must obey the shared AIOS `bidirectional-turn-compression` metric: run live work through the AIOS-managed runner, require `pre_send` compression before model input, require `post_receive` compression after model output, and treat direct host bypass as a policy violation.
